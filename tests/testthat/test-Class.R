...
test_that("Validity checks for bl class work as expected", {
  ...
  expect_error(bl(name = c('All', 'Another'), varnames = c('A'), expectation = c(0), covariance = 1),
               regexp = "Slot 'name' must have length 1, not 2.")

  # Test for unique variable names
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'A'), expectation = c(0,1,6), covariance = valid_cov),
               regexp = "All variable names in 'varnames' must be unique.")

  # Test for correct number of expectations
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'F'), expectation = c(0,1), covariance = valid_cov),
               regexp = "Slot 'expectation' has length 2. It should match the number of variables: 3.")

  # NEW: Test for NAs in expectation
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'F'), expectation = c(0, NA, 6), covariance = valid_cov),
               regexp = "The 'expectation' vector must not contain NA values.")

  # NEW: Test for NAs in covariance
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'F'), expectation = c(0, 1, 6), covariance = matrix(c(1, 2, NA, 4), 2, 2)),
               regexp = "The 'covariance' matrix or value must not contain NA values.")

  # Test for covariance matrix dimensions
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'F'), expectation = c(0,1,6), covariance = matrix(1, 2, 2)),
  ...
  expect_error(bl(name = 'All', varnames = c('A', 'B', 'F'), expectation = c(0,1,6), covariance = asymmetric_cov),
  ...
  expect_error(bl(name = 'All', varnames = c('A', 'B'), expectation = c(0,1), covariance = non_psd_cov),
...
test_that("Scalar covariance validity checks for bl class", {
  ...
  expect_no_error(bl(name = 'Single', varnames = 'A', expectation = 1, covariance = 4))

  # Test for negative scalar variance
  expect_error(bl(name = 'Single', varnames = 'A', expectation = 1, covariance = -2),
               regexp = "The scalar covariance \\(variance\\) must be non-negative.")

  # Test for vector covariance when only one variable is present
  expect_error(bl(name = 'Single', varnames = 'A', expectation = 1, covariance = c(1, 2)),
...
test_that("Validity checks for bl_data class", {
  # Test for name length
  expect_error(bl_data(name = c('Data', 'Another'), varnames = c('A','B','F'), values = c(1,2,10)),
               regexp = paste0("Slot 'name' must have length 1, not 2."))

  # Test for unique variable names
  expect_error(bl_data(name = 'Data', varnames = c('A','B','A'), values = c(1,2,10)),
               regexp = "All variable names in varnames must be unique.")

  # Test for correct number of values
  expect_error(bl_data(name = 'Data', varnames = c('A','B','F'), values = c(1,2)),
