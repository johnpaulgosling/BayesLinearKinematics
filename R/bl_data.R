...
bl_data <- setClass("bl_data",
  ...
  validity = function(object) {
    ...
    if (length(object@name) != 1) {
      msg <- paste0("Slot 'name' must have length 1, not ",
                    length(object@name), ".")
      errors <- c(errors, msg)
    }
    ...
    if (length_vars != length(object@values)) {
      msg <- paste0(
        "Slot 'values' has length ", length(object@values),
      ...
      if (any(!is.finite(object@values))) {
        ...
        msg <- paste0(
          "All 'values' must be finite (i.e. not NA, Inf, or -Inf). ",
          "Problem found at indices: ",
          paste(non_finite_idx, collapse = ", "), "."
        )
        errors <- c(errors, msg)
      }
    }
...
#' object allowing control over the number of digits shown. Called automatically
#' when an object of class 'bl_data' is printed to the console.
#'
