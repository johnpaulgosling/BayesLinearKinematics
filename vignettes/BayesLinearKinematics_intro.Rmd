...
# Load the package itself. This makes its functions available within the vignette.
library(BayesLinearKinematics)

# Important Note on Dependencies:
# Ensure that any packages your functions rely on (like MASS for ginv)
# are listed in the 'Imports:' field of your package's DESCRIPTION file.
# This ensures they are available when the vignette is built.
# You generally shouldn't need to call library() for dependencies here
# if they are correctly declared in DESCRIPTION.
```
...
Suppose we observe values for some of the variables. We represent this using the `bl_data` class. Let's say we observed 'x' to be 1.1 and 'y' to be 2.1.
...
* `E_adj[X] = E[X] + Cov(X, D) Var(D)^(-1) (d - E[D])`
* `Var_adj[X] = Var[X] - Cov(X, D) Var(D)^(-1) Cov(D, X)`
where X are the variables in `bl_prior`, D are the variables in `observed_data`, and d are the observed values.
